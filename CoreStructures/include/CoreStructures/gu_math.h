...
namespace gsl {
	...
	enum gu_bit {gu_zero=0x00, gu_one=0x01};
...
{
	// perform spherical linear interpolation between p0 and p1 over parameter t where t = [0, 1] and return the resulting interpolated point.  If fabs(p0.p1) = 1.0f then slerp reduces to linear interpolation.  It is assumed ||p0|| = ||p1|| = 1.0 (may change this)
	template <typename T>
	T slerp(const T& p0, const T& p1, const float t) {
		...
		// note: this assumes vectors p0 and p1 are unit length
		float cos_theta = dotProduct(p0, p1); // note: replace with dot() in Kore
	
		float c0, c1;

		// if cos_theta = -1.0f or 1.0f then p0 and p1 are co-linear and the slerp interpolation reduces to linear interpolation to avoid division by zero.  If cos_theta = 1.0 then p0 * (1-t) + p1 * t = p0 = p1, otherwise if cos_theta=-1.0 then p0 * (1-t) + p1 * t reduces to linear interpolation between p0 and p1 since geometrically, when cos_theta = -1.0, there are an infinite number of arcs for slerp to interpolate across
		if (fequal(fabs(cos_theta), 1.0f)) {
			...
			c0 = sinf((1.0f-t)*theta) * inv_sin_theta;
			c1 = sinf(t * theta) * inv_sin_theta;
		}
